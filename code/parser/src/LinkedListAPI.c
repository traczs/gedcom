...
bool compareElement(const void *first, const void *second);
...
/**Returns an int that tells the length of list.
Utilize an iterator and nextelement.
 *@pre List must exist, but does not have to have elements.
 *@param list Pointer to linked list dummy head.
 *@return on success: int of list length.  on failure: -1
 **/
...
void* findElement(List list, bool (*customCompare)(const void* first,const void* second), const void* searchRecord)
{
  ...
  if((elem = nextElement(&iter)) == NULL)
	...
	do{
		comp = customCompare(searchRecord,elem);
		if(comp==true)
			return elem;
	}while( (elem = nextElement(&iter)) != NULL);
...
bool compareElement(const void *first, const void *second)
{
    ...
    if (strcmp((char*)tmpName1, (char*)tmpName2) == 0){
        return true;
    }else{
        return false;
    }
